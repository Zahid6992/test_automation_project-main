...
public class Fehlerseiten_P_Zufaelligebuchstaben extends DriverTestCase {

    @Parameters({"mobileOrDesktop", "deviceName", "shop", "environment"})
    @Test()
    public void p_zufaelligeBuchstaben(String mobileOrDesktop, String deviceName, String shop, String environment) throws InterruptedException, IOException {

        String testCaseName = "Fehlerseiten_P_Zufaelligebuchstaben"  + "_"  + mobileOrDesktop + "_"  + shop + "_"  + environment ;
        extentReportConfig.configureExtentReportBeforeTest(testCaseName, mobileOrDesktop, deviceName, shop, environment);
        //Check 404 URL
        driver.get(urlMap.get(shop + " " + environment) + "/p-zueflligebuchstaben");
        Thread.sleep(3000);
